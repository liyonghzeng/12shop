...
{{--				************** 展 示 ************--}}
				...
				<div class="col s6">
					<div class="content">
						...
						<h6 class="goods_id" ><a href="">{{$v->goods_name}}</a></h6>
						<div class="price">
							${{$v->goods_price}} <span>${{$v->goods_original_price}}</span>
						</div>
						<button class="btn button-default" goods_id = {{$v->goods_id}}>{{$v->goods_desc}}</button>
					</div>
				</div>
				@endforeach
			</div>
...
<script>
	$(function (){
		$(".button-default").click(function() {
				...
				location.href = 'http://12shop.com/goodslist?goods='+ii;
			// $.ajax({
			// 	 	 method: "POST",
			// 		  url: "d",
			// 		  data: { goods_id:ii}
